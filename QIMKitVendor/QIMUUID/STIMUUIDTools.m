...
+ (NSString *)deviceUUID{
    ...
    if (!resultKey) {
        ...
        NSString *deviceUid = [UICKeyChainStore stringForKey:key
                                                     service:service];
        STIMVerboseLog(@"从KeyChain中取出来的DeviceUid : %@, key : %@, service : %@", deviceUid, key, service);
        if (deviceUid == nil) {
            STIMVerboseLog(@"从KeyChain中未取到DeviceUid，准备重新生成并写入KeyChain");
            deviceUid = [self UUID];
        }
        
        resultKey = [deviceUid copy];
    }
    STIMVerboseLog(@"最终使用的deviceUUID : %@", resultKey);
    return resultKey;
}
...
+ (BOOL)setUUID:(NSString *) deviceUid {
    ...
    if (success) {
        STIMVerboseLog(@"向KeyChain中写入DeviceUid 成功 : %@, Key: %@, service : %@", deviceUid, key, service);
    } else {
        STIMVerboseLog(@"向KeyChain中写入DeviceUid 失败 : %@, Key: %@, service : %@", deviceUid, key, service);
    }
    return success;
}
...
+ (BOOL) setUserName:(NSString *) username {
    ...
    if (username == nil || [username length] <= 0){
        ...
        STIMVerboseLog(@"当前userName为空，清空KeyChain中的userName, Key : %@, service : %@", key, service);
    ...
    if (success) {
        STIMVerboseLog(@"向KeyChain中写入UserName成功 : %@, key : %@, service : %@", username, key, service);
    } else {
        STIMVerboseLog(@"向KeyChain中写入UserName失败 : %@, key : %@, service : %@", username, key, service);
    }
    return success;
}
...
+ (NSString *) loginUserName {
    ...
    STIMVerboseLog(@"从KeyChain中取出的userName : %@, key : %@, service : %@", loginUserName, key, service);
...
+ (void)setUserAuthCookie
{
    ...
    STIMVerboseLog(@"cookie %@",myCookie.cookies);
}
...
+ (NSData *)getRequestFileUrl {
    return [STIMUUIDTools dataForKey:@"requestFileUrl"];
}
...
+ (NSData *)getNewHttpRequestUrl {
    return [STIMUUIDTools dataForKey:@"NewHttpUrl"];
}
...
+ (NSData *)getRequestUrl {
    return [STIMUUIDTools dataForKey:@"NewRequestUrl"];
}
...
+ (NSData *)getRequestDoamin {
    return [STIMUUIDTools dataForKey:@"requestDoamin"];
}
...
{
    return [STIMUUIDTools dataForKey:@"sessionList"];
}

+ (NSData *)getMyGroupList
{
    return [STIMUUIDTools dataForKey:@"MyGroupList"];
}

+ (NSData *)getRecentSharedList {
    return [STIMUUIDTools dataForKey:@"recentSharedList"];
}

+ (NSData *)getFriendList
{
    return [STIMUUIDTools dataForKey:@"FriendList"];
}
...
+ (BOOL)setData:(NSData *)data forKey:(NSString *)key
{
    ...
    if (data == nil || [data length] <= 0) {
        STIMVerboseLog(@"传入的data为空，因此清除之前的数据, Key : %@, service : %@", key, service);
        return [UICKeyChainStore removeItemForKey:key service:service];
    }
    BOOL success = [UICKeyChainStore setData:data forKey:key service:service];
    if (success) {
        STIMVerboseLog(@"向KeyChain中传入data 成功, Key : %@, service : %@", key, service);
    } else {
        STIMVerboseLog(@"向KeyChain中传入data 失败, Key : %@, service : %@", key, service);
    }
    return success;
}
...
+ (NSData *)dataForKey:(NSString *)key
{
    ...
    STIMVerboseLog(@"从KeyChain中取出数据 , key : %@, service : %@", key, service);
    return data;
}
