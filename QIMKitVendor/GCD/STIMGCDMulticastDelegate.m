...
@interface STIMGCDMulticastDelegateNode : NSObject {
@private
...
@interface STIMGCDMulticastDelegate () {
...
@interface STIMGCDMulticastDelegateEnumerator () {
    NSUInteger numNodes;
...
@implementation STIMGCDMulticastDelegate
...
- (void)addDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue {
    ...
    STIMGCDMulticastDelegateNode *node =
            [[STIMGCDMulticastDelegateNode alloc] initWithDelegate:delegate delegateQueue:delegateQueue];
...
- (void)removeDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue {
    ...
    for (i = [delegateNodes count]; i > 0; i--) {
        STIMGCDMulticastDelegateNode *node = [delegateNodes objectAtIndex:(i - 1)];
        ...
        if (delegate == nodeDelegate) {
            if ((delegateQueue == NULL) || (delegateQueue == node.delegateQueue)) {
                // Recall that this node may be retained by a STIMGCDMulticastDelegateEnumerator.
...
- (void)removeAllDelegates {
    for (STIMGCDMulticastDelegateNode *node in delegateNodes) {
        node.delegate = nil;
#if __has_feature(objc_arc_weak) && !TARGET_OS_IPHONE
        node.unsafeDelegate = nil;
#endif
...
- (STIMGCDMulticastDelegateEnumerator *)delegateEnumerator {
    return [[STIMGCDMulticastDelegateEnumerator alloc] initFromDelegateNodes:delegateNodes];
}
...
@implementation STIMGCDMulticastDelegateNode
...
@implementation STIMGCDMulticastDelegateEnumerator
...
- (BOOL)getNextDelegate:(id *)delPtr delegateQueue:(dispatch_queue_t *)dqPtr {
    while (currentNodeIndex < numNodes) {
        STIMGCDMulticastDelegateNode *node = [delegateNodes objectAtIndex:currentNodeIndex];
        currentNodeIndex++;
