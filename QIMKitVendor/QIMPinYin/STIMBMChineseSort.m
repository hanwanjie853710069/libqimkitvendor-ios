...
@interface STIMBMChineseSort()
//用将需要排序的对象封装在STIMBMChineseSort对象中，包含排序的字符串，对象，首字母三个属性
//进行比较的字符串，
...
@implementation STIMBMChineseSort
...
+(NSMutableArray*)IndexArray:(NSArray*)stringArr{
    ...
    {
        NSString *pinyin = [((STIMBMChineseSort*)object).pinYin substringToIndex:1];
        //不同
...
+(NSMutableArray*)LetterSortArray:(NSArray*)stringArr{
    ...
    for (NSString* object in tempArray) {
        ...
        NSString *pinyin = [((STIMBMChineseSort*)object).pinYin substringToIndex:1];
        NSString *string = ((STIMBMChineseSort*)object).string;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr{
    ...
    {
        STIMBMChineseSort *chineseString = [[STIMBMChineseSort alloc]init];
        ...
        chineseString.string = [STIMBMChineseSort RemoveSpecialCharacter:chineseString.string];
...
+(NSMutableArray*)sortObjectArray:(NSArray*)objectArray Key:(NSString *)key{
    ...
    NSString *tempString;
    //拼音分组
    for (STIMBMChineseSort* object in tempArray) {
...
//        NSLog(@"STIMBMChineseSort user time in %f ms", useTime *1000.0);
    }
