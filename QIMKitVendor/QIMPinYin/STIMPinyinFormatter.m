...
#import "NSString+STIMPinYin4Cocoa.h"

@interface STIMPinyinFormatter ()
+(NSInteger)getNumericValue:(unichar)c;
+(NSInteger)indexOfChar:(int*) table ch:(unichar)c;
@end    

@implementation STIMPinyinFormatter
...
+ (NSString *)formatHanyuPinyinWithNSString:(NSString *)pinyinStr
  ...
  if ((ToneTypeWithToneMark == [outputFormat toneType]) && ((VCharTypeWithV == [outputFormat vCharType]) || (VCharTypeWithUAndColon == [outputFormat vCharType]))) {
      @throw [NSException exceptionWithName:@"Throwing a BadSTIMHanyuPinyinOutputFormatCombination exception" reason:@"tone marks cannot be added to v or u:." userInfo:nil];
  }
  if (ToneTypeWithoutTone == [outputFormat toneType]) {
     ...
     pinyinStr = [STIMPinyinFormatter convertToneNumber2ToneMarkWithNSString:pinyinStr];
  }
...
+ (NSString *)convertToneNumber2ToneMarkWithNSString:(NSString *)pinyinStr {
  ...
  if ([lowerCasePinyinStr matchesPatternRegexPattern:@"[a-z]*[1-5]?"]) {
    ...
    if ([lowerCasePinyinStr matchesPatternRegexPattern:@"[a-z]*[1-5]"]) {
        int tuneNumber = (int)[STIMPinyinFormatter getNumericValue:[lowerCasePinyinStr characterAtIndex:lowerCasePinyinStr.length -1]];
