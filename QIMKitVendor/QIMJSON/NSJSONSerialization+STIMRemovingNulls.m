...
@implementation NSJSONSerialization (STIMRemovingNulls)
...
+(id)JSONObjectWithData:(NSData *)data options:(NSJSONReadingOptions)opt error:(NSError *__autoreleasing *)error removingNulls:(BOOL)removingNulls ignoreArrays:(BOOL)ignoreArrays
{
    ...
    [JSONObject stimDB_recursivelyRemoveNullsIgnoringArrays:ignoreArrays];
    return JSONObject;
}

@end

@implementation NSMutableDictionary (STIMRemovingNulls)

-(void)stimDB_recursivelyRemoveNulls
{
    [self stimDB_recursivelyRemoveNullsIgnoringArrays:NO];
}
...
- (void)stimDB_recursivelyRemoveNullsIgnoringArrays:(BOOL)ignoringArrays
{
    ...
    for (id arrayKey in arrayKeys)
    {
        ...
        [array stimDB_recursivelyRemoveNullsIgnoringArrays:ignoringArrays];
    }
    ...
    for (id dictionaryKey in dictionaryKeys)
    {
        ...
        [dictionary stimDB_recursivelyRemoveNullsIgnoringArrays:ignoringArrays];
    }
}

@end

@implementation NSMutableArray (STIMRemovingNulls)
...
- (void)stimDB_recursivelyRemoveNullsIgnoringArrays:(BOOL)ignoringArrays
{
    ...
    {
        [containedArray stimDB_recursivelyRemoveNullsIgnoringArrays:ignoringArrays];
    }
    
    // Cascade down the dictionaries
    for (NSMutableDictionary * containedDictionary in [self objectsAtIndexes:dictionaryIndexes])
    {
        [containedDictionary stimDB_recursivelyRemoveNullsIgnoringArrays:ignoringArrays];
    }
}
